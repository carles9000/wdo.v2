...
#define VERSION_RDBMS_MYSQL				'2.1.001'
...
CLASS RDBMS_MySql FROM RDBMS
	...
	METHOD VersionName()						INLINE 'WDO V2'		
...
METHOD New( cServer, cUsername, cPassword, cDatabase, nPort, cType, lLog, bError ) CLASS RDBMS_MySql
	...
	if( ::lLog, ::d( if(lAjax, 'WDO AJAX Yes', 'WDO AJAX No' ) ), nil )		
	...
	//	Cargamos lib mysql
		...
		If ValType( ::pLib ) <> "P" 
			...
			if( ::lLog, ::d( 'WDO ' + ::cError + ' ' + cDll ), nil )
			
			IF Valtype( ::bError ) == 'B'
				if Eval( ::bError, ::cError )			
					//quit	// Version V2 threads we can't use QUIT 
				endif
			ENDIF
	...
	//	Inicializamos mysql
		...
		while ::hMySql == 0 .and. nTry < 3
			...
			nTry++
			if( ::lLog, ::d( 'WDO ' + 'MySQL library failed to initialize. Try: ' + str(nTry)), nil )
			inkey(0.1)	
			::hMySQL = ::mysql_init()			
...
METHOD d( c ) CLASS RDBMS_MySql
	...
	WAPI_OUTPUTDEBUGSTRING( mh_valtochar(c) + chr(13) + chr(10) )
	
	if ::lLogFile
		::l( mh_valtochar(c) + chr(13) + chr(10) )	
	endif 
...
METHOD l( uValue ) CLASS RDBMS_MySql

	local cFile 	:= HB_GetEnv('PRGPATH') + '/log.txt'
	local cInfo   	:= dtoc(date()) + ' ' + time() + ' ' + procname(1) + '(' +  ltrim(str(procline( 1 ))) + ')'	
	LOCAL cLine, hFile	
	...
	//	Log	
		...
		cLine  	:=  cInfo + ' [' + valtype(uValue) + '] : ' + mh_valtochar( uValue ) + Chr(13) + Chr(10)
