...
METHOD View( aData ) CLASS RecordSet
	...
	FOR n := 1 to nLen 
		...
		IF lAssociative 
			...
			FOR j := 1 to ::nFields		
				cHtml += '<td>' + mh_valtochar( HB_HValueAt( aData[n] , j )) + '</td>'	
			NEXT
			
		ELSE
		
			FOR j := 1 to ::nFields		
				cHtml += '<td>' + mh_valtochar( aData[n,j] ) + '</td>'					
			NEXT				
...
METHOD ViewStruct() CLASS RecordSet
	...
	FOR n := 1 to nLen 
		...
		cHtml := '<tr>'
			...
			cHtml += '<td>' + mh_valtochar( ::aStruct[n][1] ) + '</td>'
			cHtml += '<td>' + mh_valtochar( ::aStruct[n][2] ) + '</td>'
			cHtml += '<td>' + mh_valtochar( ::aStruct[n][3] ) + '</td>'
			cHtml += '<td>' + mh_valtochar( ::aStruct[n][4] ) + '</td>'
			cHtml += '<td>' + mh_valtochar( ::aStruct[n][5] ) + '</td>'
			cHtml += '<td>' + mh_valtochar( ::aStruct[n][6] ) + '</td>'
